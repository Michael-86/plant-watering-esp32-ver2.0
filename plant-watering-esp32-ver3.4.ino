...
// Relä utgång
const int relay = 13;
const int valveRelay = 5;  // New relay for valve control
...
char autostate[4] = "on";
char pump[4] = "off";
char valve[4] = "off";  // New valve state variable
...
void handlePumpControl() {
  ...
  digitalWrite(relay, shouldPumpRun ? LOW : HIGH);
...
function toggleCheckbox(element) {
  console.log("Toggling:", element.id, "to", element.checked);
  ...
  if(element.checked){ 
    xhr.open("GET", "/update2?output="+element.id+"&state=1", true); 
  }
  else { 
    xhr.open("GET", "/update2?output="+element.id+"&state=0", true); 
  }
  xhr.send();
}
...
<body>
  ...
  <p>
    <span class="dht-labels">Ventilens läge: </span>
...
String outputState(int output) {
  if (output == valveRelay) {
    // Invert logic for valve relay if it's active-low
    return digitalRead(valveRelay) == LOW ? "checked" : "";
  }
...
String processor(const String &var) {
  if (var == "TEMPERATURE") {
    ...
    buttons += "<h4>Ventil AV - Ventil PÅ</h4><label class=\"switch\"><input type=\"checkbox\" onchange=\"toggleCheckbox(this)\" id=\"5\" " + outputState(5) + (strcmp(pumprun, "On") == 0 ? " disabled" : "") + "><span class=\"slider\"></span></label>";
    return buttons;
  }
  return String();
}
...
void setup() {
  ...
  digitalWrite(valveRelay, HIGH); // Start with valve OFF
  strcpy(valve, "off");
  ...
  server.on("/valvestate", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send_P(200, "text/plain", String(valve).c_str());
  });
  ...
  if (request->hasParam(PARAM_INPUT_1) && request->hasParam(PARAM_INPUT_2)) {
    int pin = request->getParam(PARAM_INPUT_1)->value().toInt();
    int state = request->getParam(PARAM_INPUT_2)->value().toInt();

    // Special handling for valve relay (active-low)
    if (pin == valveRelay) {
      digitalWrite(valveRelay, state ? LOW : HIGH); // Inverted logic
      strcpy(valve, state ? "on" : "off");
    } 
    else {
